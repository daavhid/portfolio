...
import { handleFormSubmit } from '@/actions/submit'
...
const Contact = () => {
    ...
    const SubmitForm = async(formData:FormData)=>{
        try{
            ...
            const data = await handleFormSubmit(formData)
            if(data?.error){
  ...
  return (
    <section ref={ref} id='contact' className='w-[min(100%,38rem)] sm:mb-28 mb-12 mt-12 scroll-mt-28 sm:mt-40'>
        ...
        ref={divRef}>
            ...
            <form action={SubmitForm} className='flex flex-col gap-2 mt-10' >
                <input type="email" name='senderEmail' required maxLength={500} placeholder='your email' className='inline-block w-full px-4 py-5 border rounded-lg dark:text-gray-950 dark:bg-white dark:bg-opacity-70 dark:focus-within:bg-opacity-90 transition-all  dark:outline-none border-black/10 placeholder:text-slate-500'/>
